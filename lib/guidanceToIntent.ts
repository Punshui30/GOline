...
export function translateGuidanceToIntent(
  ...
  // Calculate bodyLoadPreference
  const bodyKeywords = ['body', 'physical', 'relaxed', 'muscle', 'tension'];
  const hasBodyPreference = resolvedPriorities.some(p => bodyKeywords.some(k => p.toLowerCase().includes(k)));
  const bodyLoadPreference = hasBodyPreference ? 0.7 : 0.4;

  // Map numeric overshootTolerance to union strings
  const intensityKeywords = ['peak', 'intense', 'strong', 'powerful', 'rush'];
  ...
  if (anxietySensitivity > 0.6) tol = 'low';
  else if (hasIntensityPreference) tol = 'high';

  // Map numeric duration to union strings
  let dur: 'impulse' | 'sustained' | 'extended' = 'sustained';
  const durationValue = temporal.duration || 0.5;
  if (durationValue > 0.8) dur = 'extended';
  else if (durationValue < 0.4) dur = 'impulse';
  ...
  return {
    ...
    cognitiveEndurance,
    bodyLoadPreference,
    temporalOnset: temporal.onset ?? 0.5,
